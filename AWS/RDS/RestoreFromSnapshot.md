## DERS NOTLARI ##
* MySQL Client, MySQL veritabanı sunucusuna erişim sağlayan ve veritabanı ile iletişim kurmanızı sağlayan bir yazılımdır. 
* MySQL Server veritabanını barındıran ve yöneten yazılımdır. 
* Mysql Clientın Mysql serverden farkı :Özetle, MySQL Server veritabanı hizmetini sunan sunucu yazılımıdır ve MySQL Client ise bu sunucuya erişmek ve veritabanı işlemlerini gerçekleştirmek için kullanılan istemci yazılımıdır. 




## lAUNCH EC2 INSTANCE CREATE#
* Name            : RDS_Client
  AMI             : Ubuntu 22.04
...
* Security Group:
  - SSH           -----> 22    -----> Anywhere
* bash
  sudo apt update -y --> sistemi güncelle
  sudo apt upgrade -y --> sistemdeki yazılım paketlerini güncellemek için kullanılır ve kullanıcıya her bir güncelleme işlemi için onay verme gerekliliğini kaldırır. 
* Install the `mariadb-client`. yüklüyoruz
  sudo apt-get install mariadb-client -y

## CREATE SECURİTY GROUP#
* Name: DatabaseSecGrb
  Inbound: Mysql/Aurora ------>3306------> Anywhere --> içerisini yukarıda olusturmus oldugumuz seg grubu da baglıyoruz

# CREATE RDS #
  * Standard create -->
  * MySQL -->
  * Free tier -->
  * Settings --> DB instance identifier--> AWS de beni temsil edecek isim
  * Credentials Settings --> Master username--> baglanacamız ısım
  ...
  * Public access --> no
  * Existing VPC security groups --> rds icin olusturdugumuz secgrp yazıyoruz
  * Additional configuration --> database isim yazıyoruz
# CONNECT THE RDS MYSQL DB #
  * mysql -h Your RDS Endpoint -u admin -p --> Your RDS Endpoint --> Connectivity & security --> Endpoint & port --> yasin-root.csqohhvxqpww.us-east-1.rds.amazonaws.com
  * USE clarusway; clarusway datasına giriyoruz
  * SHOW TABLES; --> tabloları goster
## CREATE A TABLE NAMED 'office' İN CLARUSWAY DATABASE ##
...
* SHOW TABLES; --> tabloları goster
* SELECT * FROM offices; --> offices datasını sec
* SELECT * FROM employees; --> employess datasını sec

## MANUAL SNAPSHOT ##
* Databases --> actions --> Take Snapshot 
* DB Instance --> onceki olusturdugumuzu gorucez
* Snapshot Name --> istedimizi yazabiliriz
* Take snapshot
## TEKRAR KONSOLA GELDİK TEST ETME AMACLI ##
* DELETE FROM employees WHERE salary > 70000; -->> maası 70000 altında olanlları secıyoruz
* 13 tane per. maası 70 altı oldugu ıcın sılındı
* Yaptıgımız islemi geri almak icin snapshota geri donebiliriz
* Manuel snapshot fotofraf makınesi
* Automated backups film gibi
## Connect to RDS created from Manuel Snapshot ## 
* burda yeni bir rds acılacak
* Snapshots --> Actions --> Restore snapshots 
* DB instance settings --> DB engine --> MySQL Commity 
* Availability and durability --> Single DB instance
* Settings --> DB snapshot ID --> aws de bunu nasıl tanımlamak ıstersek onu yazıyoruz 
* Instance configuration --> Burstable classes (includes t classes) --> db.t3.micro
* Existing VPC security groups --> rds icin olusturdugumuz sec grp
* Additional configuration --> 3306
* Restore DB Instance
* Bu islemleri yapmamızdaki amaca silmis oldugumuz verileri tekrar geri getirmek yeni bir rds yaratıldı ve bız ona baglanarak o verılerı gorucez
* manuel snapshot baglanmak icin --> mysql -h <yasin-root.csqohhvxqpww.us-east-1.rds.amazonaws.com> -u admin -p
* show databases; 
* use clarusway;
* SELECT * FROM employees; yazınca onceki veriler tekrar geri geliyor
* DELETE FROM employees WHERE salary > 60000; --> maası 6000 altınd olanları da siliyoruz
* mysql -h yasin-root.csqohhvxqpww.us-east-1.rds.amazonaws.com -u admin -p   --> roota baglanıyoruz tekradan 
* DELETE FROM employees WHERE salary > 60000; ile siliyoruz suan 4 tane kaldı 
* RDS --> Databases --> yasin-root --> actions --> Restore to point in time --> 
* Custom date and time --> kendimiz spesifik bir zaman secmek istersek
* DB instance identifier --> istedigimiz ismi veriyoruz
* Instance configuration --> Burstable classes (includes t classes) --> db.t2.micro 
* Availability and durability --> Single DB instance
* Database options --> Initial database name --> clarusway --> burada yeni bir makine ayaga kaldırıyoruz
* Restore ediyoruz
* mysql -h yasin-root.csqohhvxqpww.us-east-1.rds.amazonaws.com -u admin -p  yazıp giriyoruz
* use clarusway; --> database seciyoruz
* SELECT * FROM employees; --> 
* mysqldump -h [restored-from-point-in-time-RDS endpoint] -u admin -p clarusway > backup.sql --> baglan ve verileri backup.sq1 ındır
* mysql -h [root-RDS endpoint] -u admin -p clarusway < backup.sql --> verileri geri yüklüyoruz
* son olarak da silme islemi
